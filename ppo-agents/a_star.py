...
class AStarAgent:
    ...
    def build_grid(self, asteroids):
        ...
        grid = [[True for _ in range(self.grid_cols)] for _ in range(self.grid_rows)]
        ...
        for asteroid in asteroids:
            ...
            max_col = min(
                self.grid_cols - 1, int((ax + block_radius) // self.cell_width)
            )
            min_row = max(0, int((ay - block_radius) // self.cell_height))
            max_row = min(
                self.grid_rows - 1, int((ay + block_radius) // self.cell_height)
            )
    ...
    def find_safest_cell(self, grid, asteroids):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                for asteroid in asteroids:
                    dist = math.hypot(
                        cx - asteroid.position.x, cy - asteroid.position.y
                    )
    ...
    def find_best_powerup(self, grid, powerups, player):
        ...
        for powerup in powerups:
            ...
            if (
                powerup_cell[1] < 0
                or powerup_cell[1] >= len(grid)
                or powerup_cell[0] < 0
                or powerup_cell[0] >= len(grid[0])
            ):
            ...
            dist = math.hypot(
                powerup_cell[0] - player_cell[0], powerup_cell[1] - player_cell[1]
            )
    ...
    def a_star_search(self, grid, start_cell, goal_cell):
        ...
        if not (
            0 <= sr < rows and 0 <= sc < cols and 0 <= gr < rows and 0 <= gc < cols
        ):
            return []
    ...
    def follow_path(self, dt, player):
        ...
        dist = math.hypot(
            player.position.x - next_world_x, player.position.y - next_world_y
        )
        if dist < 10:
        ...
        desired_angle = (
            math.degrees(
                math.atan2(
                    next_world_y - player.position.y, next_world_x - player.position.x
                )
            )
            - 90
        )
